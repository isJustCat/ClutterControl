...
#[get("/?<id>&<e_type>")]
pub async fn r_get(app: &State<App>, id: Option<String>, e_type: Option<String>) -> Result<ApiResponse, ApiResponse> {
    ...
    if let Some(e_type) = e_type {
        for (uuid, stored_entity) in storage.0.iter() {
            match stored_entity {
                Entity::Creature(_) if e_type.eq_ignore_ascii_case("creature") => {
                    result.insert(*uuid, stored_entity.clone());
                }
                Entity::Item(_) if e_type.eq_ignore_ascii_case("item") => {
                    result.insert(*uuid, stored_entity.clone());
                }
                Entity::Location(_) if e_type.eq_ignore_ascii_case("location") => {
                    result.insert(*uuid, stored_entity.clone());
                }
                Entity::Change(_) if e_type.eq_ignore_ascii_case("change") => {
                    result.insert(*uuid, stored_entity.clone());
                }
                Entity::Condition(_) if e_type.eq_ignore_ascii_case("condition") => {
                    result.insert(*uuid, stored_entity.clone());
                }
                Entity::Tag(_) if e_type.eq_ignore_ascii_case("tag") => {
                    result.insert(*uuid, stored_entity.clone());
                }
                _ => {}
            }
